...
from autocontext.scenarios.custom.investigation_codegen import (
    generate_investigation_class,
)
from autocontext.scenarios.custom.investigation_designer import design_investigation
...
class InvestigationCreator:
    ...
    def create(self, description: str, name: str) -> ScenarioInterface:
        spec = design_investigation(description, self.llm_fn)
        errors = validate_for_family("investigation", asdict(spec))
        if errors:
            raise ValueError(f"investigation spec validation failed: {'; '.join(errors)}")

        custom_dir = self.knowledge_root / CUSTOM_SCENARIOS_DIR
        scenario_dir = custom_dir / name
        scenario_dir.mkdir(parents=True, exist_ok=True)

        source = generate_investigation_class(spec, name=name)
        source_errors = validate_source_for_family("investigation", source)
        if source_errors:
            raise ValueError(
                f"investigation source validation failed: {'; '.join(source_errors)}"
            )
        ...
        (scenario_dir / "scenario_type.txt").write_text(
            get_family_marker("investigation"),
            encoding="utf-8",
        )

        cls = load_custom_scenario(custom_dir, name, InvestigationInterface)
        from autocontext.scenarios import SCENARIO_REGISTRY

        SCENARIO_REGISTRY[name] = cls
        logger.info("registered investigation scenario '%s'", name)
